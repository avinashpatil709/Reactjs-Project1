      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const query = new URLSearchParams(
            Object.entries(formData).map(([key, value]) => [key, encodeURIComponent(value)])
          ).toString();
          
          // const url = `http://10.10.1.18/Santosh/email.php?${query}`;
          const url = `http://192.168.56.1/DB/email.php?${query}`;
